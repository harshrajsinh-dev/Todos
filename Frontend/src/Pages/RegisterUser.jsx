...
const RegisterUser = () => {
  ...
  const handleClick = async (data) => {
    axios.post(`${import.meta.env.VITE_BASE_URL}api/user/create`, data, {
      headers: {
      ...
      <div className='w-1/3 p-5 bg-white rounded-lg hover:shadow-lg transition-all flex flex-col gap-6'>
        <form onSubmit={handleSubmit((data) => {
          ...
          <div className="flex flex-col gap-4 ">
            <input {...register('userId', { required: true })} type="number" placeholder='User ID' className='border p-2 rounded-md' />
            <input {...register('userName', { required: true })} type="text" placeholder='User Name' className='border p-2 rounded-md' />
